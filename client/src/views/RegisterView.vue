...
async function register() {
  if (name.value.length === 0 || name.value.length > nameMaxLength || name.value.length < nameMinLength) {
    nameInput.value?.focus();
  } else if (email.value.length === 0 || email.value.length > emailMaxLength || email.value.length < emailMinLength) {
    emailInput.value?.focus();
  } else if (password.value.length === 0 || password.value.length > passwordMaxLength || password.value.length < passwordMinLength) {
    passwordInput.value?.focus();
  } else {
    try {
      await AuthenticationService.register({ name: name.value, email: email.value, password: password.value });
